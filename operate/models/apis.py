...
async def call_gemini_pro_vision_with_ocr(messages, objective, model):
    ...
    try:
        time.sleep(1)
        model_client = config.initialize_google()
...
async def call_gemini_pro_vision_with_som(messages, objective, model):
    ...
    try:
        ...
        # Convert base64 labeled image back to PIL Image for Gemini
        img_labeled_bytes = base64.b64decode(img_base64_labeled)
        img_labeled = Image.open(io.BytesIO(img_labeled_bytes))

        # Generate content using Gemini
        response = model_client.generate_content([full_prompt, img_labeled])
